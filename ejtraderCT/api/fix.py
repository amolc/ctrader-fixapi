...
class FIX:
    class Message:
        def __init__(self, sub: Optional[SubID] = None, msg_type: Optional[str] = None, parent=None):
            self.fields = []
            if parent:
                self.origin = True
        ...
        def __getitem__(self, item):
            # Handle both Field enum and string/int keys
            for k, v in self.fields:
                if k == item:
                    return v
                # Also try string comparison for numeric fields
                if isinstance(item, str) and str(k) == item:
                    return v
                if isinstance(item, int) and k == item:
                    return v
            return None

        def get(self, item, default=None):
            """Dict-like get method for field access"""
        ...
        def __bytes__(self):
            data = bytearray()
            for k, v in self.fields:
                key_str = str(k.value) if hasattr(k, 'value') else str(k)
                data.extend(b"%b=%b\x01" % (key_str.encode(), str(v).encode()))
            if self.origin:
        ...
        def __repr__(self):
            return pformat([((k.name if hasattr(k, 'name') else str(k)), v) for k, v in self.fields])
    ...
    def __init__(
        ...
        try:
            ...
            # Initialize trade capture tracking
            self.trade_capture_evt = threading.Event()
            self.trade_capture_data = []
    ...
    def process_exec_report(self, msg):
        exec_type = msg[Field.ExecType]
        pos_id = msg[Field.PosMaintRptID]
        cl_ord_id = msg[Field.ClOrdId]
        
        # Debug: Log all ExecutionReports
        logging.info(f"ExecutionReport: ExecType={exec_type}, pos_id={pos_id}, cl_ord_id={cl_ord_id}")
        
        # Capture TransactTime from ALL ExecutionReports (if available)
        # This includes responses from OrderMassStatusRequest (35=AF) with ExecType="I"
        try:
            transact_time = msg["60"]  # TransactTime - actual trade execution time
            if transact_time and pos_id:
                if not hasattr(self, 'position_open_times'):
                    self.position_open_times = {}
                self.position_open_times[pos_id] = transact_time
                logging.info(f"ExecutionReport: Captured TransactTime for pos_id={pos_id}: {transact_time}")
        except Exception as e:
            logging.debug(f"ExecutionReport: No TransactTime for pos_id={pos_id} - {e}")
        
        if exec_type == "F":  # F = Trade (fill)
            self.position_list = {}
            self.position_request()
            self.order_list = {}
            self.origin_to_pos_id[cl_ord_id] = pos_id
            self.order_request()
        elif exec_type in ["0", "4", "5", "C"]:
            logging.info(f"ExecutionReport: ExecType={exec_type} (order status), pos_id={pos_id}")
            self.order_list = {}
            self.order_request()
        elif exec_type == "I":
            logging.info(f"ExecutionReport: ExecType={exec_type} (order status), pos_id={pos_id}")
    ...
    def process_market_data(self, msg: Message):
        symbol_val = msg[Field.Symbol]
        if symbol_val is None:
            return
        name = self.sec_id_table[int(symbol_val)]["name"]
        digits = self.sec_id_table[int(symbol_val)]["digits"]
    ...
    def process_sec_list(self, msg):
        logging.info("process_sec_list: Received SecurityList (35=y)")
        sec_list = msg.get_repeating_groups(Field.NoRelatedSym, Field.Symbol)
        logging.info(f"process_sec_list: Processing {len(sec_list)} symbols")
        ...
        cb = self.sec_list_callback
        if cb:
            cb()
        self.position_request()
        self.order_request()
        # Wait for ExecutionReports from OrderMassStatusRequest to arrive
        time.sleep(2)
    ...
    def process_position_list(self, msg):
        if msg[Field.PosReqResult] == "2":
            return
        name = self.sec_id_table[int(msg[Field.Symbol])]["name"]
        # Note: PositionReport doesn't contain position open time
        # We get actual trade time from ExecutionReport or TradeCaptureReport
        ...
        if name not in self.spot_request_list:
            # print(self.spot_market_request)
            self.spot_market_request(name)
        # base = name[-3:]
        # print("===base====", base)
        # if base != self.currency:
        #     pair = "%s%s" % (base, self.currency)
        #     print("===pair==self.currency==", pair)
        #     conv_dir = 0
    ...
    def process_reject(self, msg):
        ...
        if text:
            ...
            if len(parts) > 1:
                ...
                logging.error(f"Reject: {text}")
        else:
            logging.error("Reject message received without Text field")
    ...
    def process_message(self, msg: Message):
        ...
        fix_str = str(msg)
        logging.info(f"FIX IN: {fix_str}")
        msg_type = msg[Field.MsgType]
        if msg_type is not None:
            FIX.message_dispatch[msg_type](self, msg)

    def send_message(self, msg: Message):
        # Log FIX message being sent
        fix_str = str(msg)
        logging.info(f"FIX OUT: {fix_str}")
    ...
    def qheartbeat(self, test_id: Optional[int] = None):
        msg = FIX.Message(SubID.QUOTE, "0", self)
        if test_id:
            msg[Field.TestReqID] = test_id
        self.send_message(msg)

    def theartbeat(self, test_id: Optional[int] = None):
    ...
    def market_request(self, subid, symbol, callback):
        ...
        sym_val = msg[Field.Symbol]
        if sym_val is not None:
            self.subscribed_symbol[subid] = int(sym_val)
        self.market_callback = callback
        self.send_message(msg)
        self.market_seq += 1

    def spot_market_request(self, symbol):
        logging.info(f"====spot_market_request")
        logging.info(symbol)
        # if symbol == 'EK50':
        #     symbol = 'HK50' 
        msg = FIX.Message(SubID.QUOTE, "V", self)
    ...
    def order_request(self):
        logging.info("order_request: Sending OrderMassStatusRequest (35=AF)")
        msg = FIX.Message(SubID.TRADE, "AF", self)
        msg[Field.MassStatusReqID] = self.client_id
        msg[Field.MassStatusReqType] = 7
        self.send_message(msg)
        logging.info("order_request: OrderMassStatusRequest sent")

    def sec_list(self, callback=None):
        logging.info("sec_list: Sending SecurityListRequest (35=x)")
        ...
        self.send_message(msg)
        logging.info("sec_list: SecurityListRequest sent")
    ...
    def close_position(self, pos_id: str, lots=None, symbol_name=None, side=None):
        logging.info(f"Closing position {pos_id} with lots {lots}")
        
        if str(pos_id) in self.position_list:
             pos_id = str(pos_id)
        
        pos_data = self.position_list.get(pos_id)

        if not pos_data:
            if symbol_name and side:
                logging.info(f"Position {pos_id} not found in list, using provided fallback: {symbol_name}, {side}")
            else:
                logging.warning(f"Position {pos_id} not found in position_list")
                return
        ...
        if pos_data:
            msg[Field.Symbol] = self.sec_name_table[pos_data["name"]]["id"]
            msg[Field.Side] = (
                Side.Sell.value
                if pos_data["long"] > 0
                else Side.Buy.value
             ...
             msg[Field.Symbol] = self.sec_name_table[symbol_name]["id"]
             # If position side is Buy, we Sell to close.
             msg[Field.Side] = (
                Side.Sell.value
                if side == "Buy"
                else Side.Buy.value
