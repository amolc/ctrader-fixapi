...
class Ctrader:
    ...
    def trade(
        ...
        if v_action in ["CLOSED", "PCLOSED"]:
            size = float(v_lots)
        else:
            # Calculate size based on symbol's pip_position if available
            pip_position = 4  # Default to 4 for FX symbols
            for _, sym_info in self.symbol_table.items():
                if sym_info['name'] == symbol:
                    pip_position = sym_info['pip_position']
                    break
            
            # For BTCUSD (pip_position=1), 0.01 lots should be 0.1 units (volume * 10^1)
            # EURUSD (pip_position=4), 0.01 lots should be 100 units (volume * 10^4)
            # This resolves the issue where 0.01 BTCUSD was converted to 1000 units (volume * 10^5)
            # which exceeded the 5 unit limit. The user requested 0.01 lots to be 0.1 units.
            raw_size = round(float(v_lots) * (10 ** pip_position), 8)
            
            size = int(raw_size) if raw_size.is_integer() else raw_size
        ...
        if v_action == "OPEN":
            ...
            if int(v_type) > 1:
                ...
                # POSICAO
                self.fix.close_position(v_ticket, size, symbol_name=v_symbol, side=side)
                # cancela ordens pendentes abertas de TP e SL
                ticket_orders = self.getOrdersIdByOriginId(v_ticket, client_id)
                self.cancelOrdersByOriginId(ticket_orders, client_id)
                if command:
    ...
    def positionCloseById(self, id, amount, symbol=None, side=None):
        try:
            sym = symbol if symbol else ""
            action = self.trade(sym, "CLOSED", 0, "", amount, 0, 0, 0, 5, id, side=side)
        except Exception as e:
            logging.info(e)
            action = None
            pass
    ...
    def position_list_callback(self, data: dict, price_data: dict, client_id: str):
        ...
        for i, kv in enumerate(data.items()):
            ...
            if price:
                ...
                convert = kv[1].get("convert")
                convert_dir = kv[1].get("convert_dir")
